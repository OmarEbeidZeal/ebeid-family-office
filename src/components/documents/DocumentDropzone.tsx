...
import { useQueueDocuments, type DocumentUploadProgress } from "@/hooks/useDocuments";
import { DOCUMENT_UPLOAD_ACCEPT } from "@/lib/documents/types";
...
/**
 * One way in for everything on paper.
 *
 * Nothing is asked before the upload — not the type, not the person. The reader
 * works both out, and the shelf asks only where an answer would otherwise be a
 * guess with consequences.
 */
export function DocumentDropzone({ onQueued }: { onQueued?: (batchId: string) => void }) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState(false);
  const [progress, setProgress] = useState<DocumentUploadProgress[]>([]);
  const queue = useQueueDocuments();

  // ⌘K → "Upload paperwork" lands here and opens the file chooser straight away.
  useQuickAdd("document", () => inputRef.current?.click());
  ...
  const send = async (list: FileList | File[]) => {
    ...
    try {
      ...
      toast.error("Those files could not be queued", {
    ...
    <div>
      ...
      >
        ...
        <p className="text-sm font-medium text-foreground">
          {busy ? "Uploading…" : "Drop anything financial here"}
        </p>
        <p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed text-muted-foreground">
          Policy schedules, tenancy agreements, payslips and bank statements. Up to 20MB each and 40
          at a time. The type is recognised from the file itself; a statement is handed straight to
          the importer. National Insurance numbers are never stored, and policy, payroll and tenancy
          references are kept only as their last four characters.
        </p>
